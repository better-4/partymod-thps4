...
static in_addr_t gsnatneg_gsresolv(uint8_t *gamename, int num) {
    ...
    if(num > 0) {   // natneg
        if(gamename) len = snprintf(tmp, sizeof(tmp), "%s.natneg%d.openspy.net", gamename, num);
        else         len = snprintf(tmp, sizeof(tmp), "natneg%d.openspy.net", num);
    } else if(num < 0) {
        if(gamename) len = snprintf(tmp, sizeof(tmp), "%s.master.openspy.net", gamename);
        else         len = snprintf(tmp, sizeof(tmp), "master.openspy.net");
    } else {    // ms?
        if(gamename) len = snprintf(tmp, sizeof(tmp), "%s.ms%d.openspy.net", gamename, server_num);
        else         len = snprintf(tmp, sizeof(tmp), "master.openspy.net");
    }
...
#define GSNATNEG_MYCALLBACK \
    ...
    */
    ...
    if(
        ...
        gsmsx     = gsnatneg_gsresolv(gamename, 0);     // gamename.ms?.openspy.net
        if(gsmsx     == INADDR_NONE) GSNATNEG_GOTOQUITX
        gsmsh     = gsnatneg_gsresolv(gamename, -1);    // gamename.master.openspy.net
        if(gsmsh     == INADDR_NONE) GSNATNEG_GOTOQUITX
        gsnatneg1 = gsnatneg_gsresolv(gamename, 1);     // gamename.natneg1.openspy.net
        if(gsnatneg1 == INADDR_NONE) GSNATNEG_GOTOQUITX
        gsnatneg2 = gsnatneg_gsresolv(gamename, 2);     // gamename.natneg2.openspy.net
...
redo:
    ...
    for(;;) {
        if((uint32_t)time(NULL) >= (uint32_t)send_natneg) {
            if(client) {
                st = gsnatneg_make_tcp();   // connect to gamename.ms?.openspy.net
