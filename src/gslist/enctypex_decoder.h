...
#ifndef _ENCTYPEX_DECODER_C_
#define _ENCTYPEX_DECODER_C_
...
unsigned char *enctypex_msname(unsigned char *gamename, unsigned char *retname) {
    ...
    if(retname) {
        snprintf(retname, 256, "%s.ms%d.openspy.net", gamename, server_num);
        return(retname);
    }
    snprintf(msname, sizeof(msname), "%s.ms%d.openspy.net", gamename, server_num);
    return(msname);
}
