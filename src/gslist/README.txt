...
The gslist source code distributed with the Better4 project has been heavily modified from its
original version. Specifically, it has been converted to a header-only C library and much of the
miscellaneous functionality (SQL, web, GeoIP, etc.) has been stripped out for the convenience of
having less dependencies. In its current state, it exposes a single barebones "gslist" function
which returns information about all OpenSpy servers.

See LICENSE for more details about using, modifying, and distributing derivative works of gslist.
The original README.txt has been preserved below for posterity.
